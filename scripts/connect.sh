...
function setInstanceIdandAz {
    ...
    result=$(aws ec2 describe-instances --filter "Name=tag:JumpboxFor,Values=db" --query "Reservations[].Instances[?State.Name == 'running'].{Id:InstanceId, Az:Placement.AvailabilityZone}[]" --output text)
